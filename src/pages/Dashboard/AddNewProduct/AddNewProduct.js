  ...
  const onSubmit = (data) => {
    axios.post("https://fast-beach-64990.herokuapp.com/foods", data).then((res) => {
      if (res.data.insertedId) {
        ...
        reset();
        setSuccess("Food Package Added Successfully !");
      }
    });
  };
  ...
  return (
    <div className="pt-5">
      <h3 className="text-center text-dark mb-5 text-uppercase">
        add a new <span style={{ color: "#ea3c23" }}>Food Package's</span>
      </h3>
      ...
      >
        ...
        <Button className="w-75" variant="warning" type="submit">
          Add Package
        </Button>
      </Form>
      <p className="text-center text-warning text-uppercase mt-4 mb-5 pb-5 fs-4">
        {success}
      </p>
    </div>
  );
